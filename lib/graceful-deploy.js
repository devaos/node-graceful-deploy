...
function gracefulDeploy() {
  ...
  this.on('finished', function() {
    this.isSkinjob = false
    human.ohSnapImASkinjob(skinjob.servers)
    skinjob.remAllServers()
  })
...
gracefulDeploy.prototype.bind = function(s) {
  ...
  /*
   */
  bound.listen = function(port, hostname, backlog, callback) {
    bound.port = port
    ...
    try {
      bound.instance.listen(port, hostname, backlog, function() {
